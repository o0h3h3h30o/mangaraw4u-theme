...
import { adminApi, type AdminAuthor } from "@/lib/api/endpoints/admin";
...
export default function AdminAuthorsPage() {
  ...
  const [editItem, setEditItem] = useState<AdminAuthor | null>(null);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const data = await adminApi.listAuthors(search || undefined);
      setAuthors(data);
    } catch (err) {
  ...
  const openEdit = (item: AdminAuthor) => {
    setEditItem(item);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editItem) {
        await adminApi.updateAuthor(editItem.id, { name: formName, slug: formSlug });
        toast.success("Author updated");
      } else {
        await adminApi.createAuthor({ name: formName, slug: formSlug });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Authors ({authors.length})</h1>
        <Button onClick={openCreate}><Plus className="h-4 w-4 mr-2" /> Add Author</Button>
      </div>
                  ...
                  <tr key={a.id} className="border-b hover:bg-muted/30">
                    ...
                    <td className="p-3 text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        <Button variant="ghost" size="sm" className="text-destructive" onClick={() => setDeleteId(a.id)}><Trash2 className="h-3.5 w-3.5" /></Button>
                      </div>
                    </td>
                  </tr>
                ))}
                {authors.length === 0 && (
                  <tr><td colSpan={4} className="p-6 text-center text-muted-foreground">No authors found</td></tr>
                )}
              </tbody>
            </table>
          )}
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editItem ? "Edit Author" : "New Author"}</DialogTitle></DialogHeader>
      ...
      <Dialog open={deleteId !== null} onOpenChange={() => setDeleteId(null)}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">This will permanently delete this author and remove it from all manga. This action cannot be undone.</p>
          <DialogFooter>
